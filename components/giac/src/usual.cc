...
//#endif
  ...
  const define_alias_gen(alias_rad2deg_g,_DOUBLE_, static_cast<signed char>((*(ulonglong *)&rad2deg_d) >> 8),(*(ulonglong *)&rad2deg_d)>>16);
  const gen & rad2deg_g = *(const gen*) & alias_rad2deg_g;
  const define_alias_gen(alias_deg2rad_g,_DOUBLE_, static_cast<signed char>((*(ulonglong *)&deg2rad_d) >> 8),(*(ulonglong *)&deg2rad_d)>>16);
  const gen & deg2rad_g = *(const gen*) & alias_deg2rad_g;
  //grad
  const define_alias_gen(alias_rad2grad_g,_DOUBLE_, static_cast<signed char>((*(ulonglong *)&rad2grad_d) >> 8),(*(ulonglong *)&rad2grad_d)>>16);
  const gen & rad2grad_g = *(const gen*) & alias_rad2grad_g;
  const define_alias_gen(alias_grad2rad_g, _DOUBLE_, static_cast<signed char>((*(ulonglong *)&grad2rad_d) >> 8), (*(ulonglong *)&grad2rad_d) >> 16);
